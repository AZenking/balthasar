...
| `POSTGRES_BIND` | `127.0.0.1` | 端口绑定地址,`0.0.0.0` 时对外 | ✓ |
...
| 变量 | 默认 | 影响 | 改后重启? |
|---|---|---|---|
| `ALLOW_REGISTRATION` | `false` | `true` 时允许新用户注册 (首用户例外,详见 [operations.md](./operations.md#注册管理)) | ✓ |
| `NEXT_PUBLIC_ALLOW_REGISTRATION` | 同 `ALLOW_REGISTRATION` | 登录页是否显示"注册"链接 | ✓ (client bundle 重编译) |
