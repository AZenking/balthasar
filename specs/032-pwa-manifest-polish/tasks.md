...
- [X] T007 [P] 在 `src/tests/unit/pwa/manifest.test.ts` **新增 screenshots 断言**(契约 C4):`screenshots` 数组长度 ≥ 2,至少一条 `form_factor: "narrow"` + 一条 `"wide"`
...
- [X] T025 [US4] **人工制图(NEEDS-MANUAL)**:制作 3 张截图放 `public/pwa/screenshots/`:
...
- [X] T026 [US4] 改 `public/manifest.webmanifest`:加 `screenshots` 数组(契约 C4),3 条对应 T025 的文件,每条含 `src`/`sizes`/`type`/`form_factor`/`label`
- [X] T027 [US4] 跑 `pnpm test:unit src/tests/unit/pwa/manifest.test.ts` 确认 screenshots 断言(T007)**转绿**
...
- [X] T036 [P] 文档同步:若 `docs/AGENTS.md` 或 `docs/THEME.md` 有 manifest 相关描述,核对是否需更新(深色 background_color 值)
