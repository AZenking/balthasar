...
- **排序冲突**: 多个分类 sortOrder 相同时,次级排序按 `createdAt ASC` (创建时间早的在前)。前端拖拽排序时批量更新 sortOrder,服务端在事务内保证同级 sortOrder 唯一性可选 (MVP 不强制唯一,允许冲突,按 createdAt 兜底)。
...
- **FR-004**: `icon` MUST ∈ 内置 emoji 库白名单 (前端 + 后端共享一份常量列表,约 100-200 个常用 emoji);非白名单值返回 400。
- **FR-005**: `parentId` 可选,若提供则 MUST 满足: (a) 存在且属于当前家庭或为内置;(b) `parentId` 指向的分类本身 `parentId IS NULL` (即必须是顶级);(c) 不允许循环引用 (parentId ≠ self)。违反任一返回 400。
...
- **FR-013**: 自定义分类的 `type` 字段: 若该分类**已被交易引用** (transactions 表存在 `categoryId = this.id`),则 MUST 拒绝切换 type (400 "已被引用的分类不可切换 type");未被引用时允许切换。
...
- **FR-017**: 归档父分类时 MUST 级联归档其所有子分类 (子分类因失去父失去意义);反归档父时 MUST 同步反归档子。该级联在单一事务内完成。
...
- **EmojiLibrary** (常量,非持久化): 内置 emoji 白名单,前后端共享的常量列表,约 100-200 个常用 emoji (🍔🚗💰💻🐾...)。
...
- 图标: 仅允许内置 emoji 库白名单 (约 100-200 个常用 emoji),前后端共享常量列表。不实现图片上传。
- 排序: 同级分类按 `sortOrder ASC, createdAt ASC` (003 已有 sortOrder,本 feature 沿用)。默认 sortOrder=100,前端拖拽排序批量更新。
